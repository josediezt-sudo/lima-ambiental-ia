...
por distrito (los 44 gobiernos locales de Lima Metropolitana: 1 provincial + 43
distritales — usa el nombre exacto de la tabla `distritos`, p.ej. "San Isidro" o
"Lima Metropolitana" para el ámbito provincial).
...
Uso:
    python -m src.slga.cli distritos
    python -m src.slga.cli instrumento-add --distrito "San Isidro" --tipo agenda_ambiental_local --nombre "Agenda Ambiental Local 2026-2027" --estado vigente
    python -m src.slga.cli instrumento-list [--estado vigente] [--distrito "San Isidro"]
    python -m src.slga.cli cam-integrante-add --distrito "San Isidro" --institucion "Municipalidad" --sector publico --representante "..." --cargo "..."
    python -m src.slga.cli cam-integrante-list [--distrito "San Isidro"]
    python -m src.slga.cli cam-sesion-add --distrito "San Isidro" --fecha 2026-03-15 --tipo ordinaria --asistentes 12 --acuerdos "..."
    python -m src.slga.cli cam-sesion-list [--anio 2026] [--distrito "San Isidro"]
    python -m src.slga.cli planefa-add --distrito "San Isidro" --anio 2026 --unidad "Comercio ambulatorio" --tipo-accion supervision --trimestre 2
    python -m src.slga.cli planefa-list [--anio 2026] [--distrito "San Isidro"]
    python -m src.slga.cli gals-indicador-add --distrito "San Isidro" --dimension calidad_ambiental --indicador "..." --nivel-objetivo "GALS I" --avance-pct 60 --fecha-corte 2026-06-30
    python -m src.slga.cli gals-indicador-list [--dimension calidad_ambiental] [--distrito "San Isidro"]
...
def _resolver_distrito_id(conn, nombre_distrito: str) -> int:
    distrito = resolver_distrito_por_nombre(cargar_distritos(conn), nombre_distrito)
    if distrito is None:
        print(f"Distrito no reconocido: '{nombre_distrito}'. Usa 'python -m src.slga.cli distritos' "
              "para ver los nombres válidos.")
        sys.exit(1)
    return distrito["id"]


def _cmd_distritos(conn, args):
    _print_filas(cargar_distritos(conn))


def _cmd_instrumento_add(conn, args):
    distrito_id = _resolver_distrito_id(conn, args.distrito)
    instrumento_id = repo.insertar_instrumento(
        conn, distrito_id=distrito_id, tipo=args.tipo, nombre=args.nombre, estado=args.estado,
...
def _cmd_instrumento_list(conn, args):
    distrito_id = _resolver_distrito_id(conn, args.distrito) if args.distrito else None
    _print_filas(repo.listar_instrumentos(conn, estado=args.estado, distrito_id=distrito_id))


def _cmd_cam_integrante_add(conn, args):
    distrito_id = _resolver_distrito_id(conn, args.distrito)
    integrante_id = repo.insertar_cam_integrante(
        conn, distrito_id=distrito_id, institucion=args.institucion, sector=args.sector,
        representante=args.representante, cargo=args.cargo, fecha_incorporacion=args.fecha_incorporacion,
    )
...
def _cmd_cam_integrante_list(conn, args):
    distrito_id = _resolver_distrito_id(conn, args.distrito) if args.distrito else None
    _print_filas(repo.listar_cam_integrantes(conn, solo_vigentes=not args.todos, distrito_id=distrito_id))


def _cmd_cam_sesion_add(conn, args):
    distrito_id = _resolver_distrito_id(conn, args.distrito)
    sesion_id = repo.insertar_cam_sesion(
        conn, distrito_id=distrito_id, fecha=args.fecha, tipo=args.tipo, asistentes=args.asistentes,
...
def _cmd_cam_sesion_list(conn, args):
    distrito_id = _resolver_distrito_id(conn, args.distrito) if args.distrito else None
    _print_filas(repo.listar_cam_sesiones(conn, anio=args.anio, distrito_id=distrito_id))


def _cmd_planefa_add(conn, args):
    distrito_id = _resolver_distrito_id(conn, args.distrito)
    accion_id = repo.insertar_planefa_accion(
        conn, distrito_id=distrito_id, anio=args.anio, unidad_fiscalizable=args.unidad,
        tipo_accion=args.tipo_accion, trimestre_programado=args.trimestre, estado=args.estado,
...
def _cmd_planefa_list(conn, args):
    distrito_id = _resolver_distrito_id(conn, args.distrito) if args.distrito else None
    _print_filas(repo.listar_planefa_acciones(conn, anio=args.anio, distrito_id=distrito_id))


def _cmd_gals_indicador_add(conn, args):
    distrito_id = _resolver_distrito_id(conn, args.distrito)
    indicador_id = repo.insertar_indicador_gals(
        conn, distrito_id=distrito_id, dimension=args.dimension, indicador=args.indicador,
        nivel_objetivo=args.nivel_objetivo, avance_pct=args.avance_pct, fecha_corte=args.fecha_corte,
    )
...
def _cmd_gals_indicador_list(conn, args):
    distrito_id = _resolver_distrito_id(conn, args.distrito) if args.distrito else None
    _print_filas(repo.listar_indicadores_gals(conn, dimension=args.dimension, distrito_id=distrito_id))
...
def _cmd_seed_ejemplo(conn, args):
    ...
    def con_distrito_id(item: dict) -> dict:
        item = dict(item)
        nombre_distrito = item.pop("distrito")
        distrito = resolver_distrito_por_nombre(distritos, nombre_distrito)
        if distrito is None:
            raise ValueError(f"Distrito no reconocido en {args.archivo}: '{nombre_distrito}'")
        item["distrito_id"] = distrito["id"]
        return item

    for item in datos.get("instrumentos", []):
        repo.insertar_instrumento(conn, **con_distrito_id(item))
    for item in datos.get("cam_integrantes", []):
        repo.insertar_cam_integrante(conn, **con_distrito_id(item))
    for item in datos.get("cam_sesiones", []):
        repo.insertar_cam_sesion(conn, **con_distrito_id(item))
    for item in datos.get("planefa_acciones", []):
        repo.insertar_planefa_accion(conn, **con_distrito_id(item))
    for item in datos.get("indicadores_gals", []):
        repo.insertar_indicador_gals(conn, **con_distrito_id(item))
...
def construir_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser("distritos", help="lista los 44 gobiernos locales disponibles").set_defaults(func=_cmd_distritos)
